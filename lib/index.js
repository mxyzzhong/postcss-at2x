...
export default postcss.plugin('postcss-atx', atNx);
  ...
  return function (root, result) {
    ...
    root.walkRules((rule) => {
      ...
      rule.walkDecls(/^background/, (decl) => {
        ...
        retinaImages.forEach((retinaImage, i) => {
          ...
          const promise = getBackgroundImageSize(
            ...
            .then((size) => {
              return addRetinaRule.bind(this, defaultResolutions[i], ruleContainer, mediaParent, decl, retinaImage, size);
            });
...
function addRetinaRule(resolutions, ruleContainer, mediaParent, decl, retinaImages, size) {
  ...
  const params = mediaParent.name === 'media' ?
    combineMediaQuery(mediaParent.params.split(/,\s*/), resolutions) :
    resolutions.join(', ');
